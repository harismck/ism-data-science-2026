...
with tab2:
    ...
    groupby = st.selectbox(
        ...
        options=["fuel_type", "inspection_station_municipality", "body_type"],
    )


    # def summarize(data, groupby):
    #     summary = (
    #         data.groupby(groupby)
    #         .agg(
    #             inspections=("vehicle_identifier", "size"),
    #             vehicles=("vehicle_identifier", "nunique"),
    #             passed=("inspection_passed", "sum"),
    #         )
    #         .reset_index()
    #     )

    #     summary["pass_rate"] = summary["passed"] / summary["inspections"]
    #     return summary
    ...
    with chart_cols[0]:
        fig = px.bar(
            ...
            y=groupby,
            orientation="h",
            title=f"Pass Rate by {groupby}",
        )
        st.plotly_chart(fig)

    with chart_cols[1]:
        fig = px.bar(
            aggregated.sort_values("inspections"),
            x="inspections",
            y=groupby,
            orientation="h",
            title=f"Inspection Count by {groupby}",
        )
        st.plotly_chart(fig)
...
with tab3:
    ...
    '''
    show_passed = st.toggle("Inspection passed", value=True)
    color = st.color_picker("Bar color", value="#1f77b4")
    fig = px.histogram(
        df[df["inspection_passed"] == show_passed],
        x="vehicle_age",
